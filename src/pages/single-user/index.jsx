...
const useBlockUser = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, block }) => {
      // agar block true bo‘lsa → userni block qilish, false bo‘lsa → unblock qilish
...
const useFreezeUser = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, freeze }) => {
      // agar freeze true bo‘lsa → "1" qilib yuboriladi, aks holda "0"
...
const UserPage = () => {
  ...
  // Serverdan kelgan qiymatlarni state ga o‘rnatish
  useEffect(() => {
    if (user) {
      setBlock(!user.isEnabled); // isEnabled=false → blocked=true
      setFreeze(user.balanceStatus === "1"); // balanceStatus=1 → frozen
    }
  }, [user?.id]);
          ...
          <div className="flex flex-col sm:flex-row gap-6 mt-4">
            ...
            <label className="flex cursor-pointer items-center gap-2">
              <input
                ...
                className="w-5 h-5 hidden rounded border-gray-300 cursor-pointer"
              />
